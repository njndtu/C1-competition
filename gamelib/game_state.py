...
class GameState:
    ...
    def get_resource(self, resource_type, player_index = 0):
        ...
        Args:
            resource_type: MP (1) or SP (0), pass in int
    ...
    # loops thru entire map and finds friendsly turrets, returns list of coords
    def get_friendly_turrets(self):
        coords = []
        
        start = [0,13]
        for x in range(0,14):
            cur_x = start[0] + x
            for y in range(0,28-(2*x)):
                cur_y = start[1] - x
                cur = [cur_x+y, cur_y]

                unit = self.contains_stationary_unit(cur)
    ...
    # testing really
    def spawn_turret(self, coord):
        return self.attempt_spawn(TURRET, coord)

    # SP
    def get_allowance(self):
        return self.get_resource(0,0) - self.to_save
